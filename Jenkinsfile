    ...
    node('deltacare') {
        ...
        stage('Deploy DEV') {
            container('helm') {
                ...
                sh "helm upgrade ${APP_NAME} ./helm --install --force --set image.tag=${APP_VERSION} --namespace ${K8S_NAMESPACE} --set ingress.hosts[0].host=${SUBDOMAIN}.${DOMAIN} --set ingress.hosts[0].paths[0].path=/"
